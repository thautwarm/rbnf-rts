# This file is automatically generated by RBNF.hs
#token=. {idint:int, colomn:int, lineno:int, val:str, name:str}
#ast=. {}
#linkedlist=a. {}
#tokens=. {offset:int}
#prim__eq::forall a. (a * a) -> bool
#prim__not__eq::forall a. (a * a) -> bool
#prim__null::forall a. a
#prim__peekable::(tokens * int) -> bool
#prim__peek::(tokens * int) -> token
#prim__match__tk::(tokens * int) -> bool * any
#prim__tk__id::str -> int
#prim__reset::(tokens * int) -> ()
#prim__cons::forall a. (a * linkedlist a) -> linkedlist a
#prim__nil::forall a. linkedlist a
#prim__to__errs::any -> linkedlist (int * str)
#prim__to__result::any -> ast
#prim__to__any::forall a. a -> any
#prim__mk__ast::forall a. (str * a) -> ast
#prim__is__null::forall a. a -> bool
#prim__is__not__null::forall a. a -> bool
#always__true::State -> bool
def lr_step_Mul(_slot_0, prim__state, prim__tokens):
    Mul_lhs_0 = _slot_0
    lcl_0 = prim__tokens.offset
    _off_0 = lcl_0
    lcl_0 = prim__tk__id("quote *")
    lcl_0 = prim__match__tk(prim__tokens, lcl_0)
    _slot_1 = lcl_0
    lcl_0 = prim__is__null(_slot_1)
    if lcl_0:
        lcl_1 = (_off_0, "quote *")
        lcl_1 = prim__cons(lcl_1, prim__nil)
        lcl_1 = prim__to__any(lcl_1)
        lcl_1 = (False, lcl_1)
        lcl_0 = lcl_1
    else:
        _tmp_1_flag = False
        lcl_1 = prim__tokens.offset
        _off_1 = lcl_1
        lcl_1 = prim__peekable(prim__tokens, 0)
        if lcl_1:
            # switch
            lcl_3 = prim__peek(prim__tokens, 0)
            lcl_3 = lcl_3.idint
            if lcl_3 == prim__tk__id("quote ("):
                lcl_3 = prim__tokens.offset
                _off_2 = lcl_3
                lcl_3 = prim__tk__id("quote (")
                lcl_3 = prim__match__tk(prim__tokens, lcl_3)
                _slot_2 = lcl_3
                lcl_3 = prim__is__null(_slot_2)
                if lcl_3:
                    lcl_4 = (_off_2, "quote (")
                    lcl_4 = prim__cons(lcl_4, prim__nil)
                    lcl_4 = prim__to__any(lcl_4)
                    lcl_4 = (False, lcl_4)
                    lcl_3 = lcl_4
                else:
                    lcl_4 = parse_Mul(prim__state, prim__tokens)
                    _slot_3_check = lcl_4
                    lcl_4 = _slot_3_check[0]
                    lcl_4 = prim__eq(lcl_4, False)
                    if lcl_4:
                        lcl_4 = _slot_3_check
                    else:
                        lcl_4 = _slot_3_check[1]
                        lcl_4 = prim__to__result(lcl_4)
                        _slot_3 = lcl_4
                        Mul_Atom_a_3 = _slot_3
                        lcl_4 = prim__tokens.offset
                        _off_3 = lcl_4
                        lcl_4 = prim__tk__id("quote )")
                        lcl_4 = prim__match__tk(prim__tokens, lcl_4)
                        _slot_4 = lcl_4
                        lcl_4 = prim__is__null(_slot_4)
                        if lcl_4:
                            lcl_5 = (_off_3, "quote )")
                            lcl_5 = prim__cons(lcl_5, prim__nil)
                            lcl_5 = prim__to__any(lcl_5)
                            lcl_5 = (False, lcl_5)
                            lcl_4 = lcl_5
                        else:
                            lcl_5 = (_slot_2, _slot_3, _slot_4)
                            _slot_local__1 = lcl_5
                            _slot_local__2 = Mul_Atom_a_3
                            Mul_rhs_4 = _slot_local__2
                            lcl_5 = (_slot_0, _slot_1, _slot_local__2)
                            _slot_local__3 = lcl_5
                            lcl_5 = mul(Mul_lhs_0, Mul_rhs_4)
                            _slot_local__4 = lcl_5
                            lcl_5 = (True, _slot_local__4)
                            lcl_4 = lcl_5
                    lcl_3 = lcl_4
                lcl_2 = lcl_3
            elif lcl_3 == prim__tk__id("number"):
                lcl_3 = prim__tokens.offset
                _off_2 = lcl_3
                lcl_4 = prim__tk__id("number")
                lcl_3 = prim__match__tk(prim__tokens, lcl_4)
                _slot_2 = lcl_3
                lcl_3 = prim__is__null(_slot_2)
                if lcl_3:
                    lcl_4 = (_off_2, "number")
                    lcl_4 = prim__cons(lcl_4, prim__nil)
                    lcl_4 = prim__to__any(lcl_4)
                    lcl_4 = (False, lcl_4)
                    lcl_3 = lcl_4
                else:
                    Mul_Atom_a_3 = _slot_2
                    lcl_4 = unwrap(Mul_Atom_a_3)
                    _slot_local__1 = lcl_4
                    Mul_rhs_3 = _slot_local__1
                    lcl_4 = (_slot_0, _slot_1, _slot_local__1)
                    _slot_local__2 = lcl_4
                    lcl_4 = mul(Mul_lhs_0, Mul_rhs_3)
                    _slot_local__3 = lcl_4
                    lcl_4 = (True, _slot_local__3)
                    lcl_3 = lcl_4
                lcl_2 = lcl_3
            else:
                _tmp_1_flag = True
                lcl_3 = prim__to__any(prim__nil)
                lcl_3 = (False, lcl_3)
                lcl_2 = lcl_3
            lcl_1 = lcl_2
        else:
            lcl_2 = prim__to__any(prim__nil)
            lcl_3 = (False, lcl_2)
            lcl_1 = lcl_3
        _tmp_1_result = lcl_1
        lcl_1 = _tmp_1_result[0]
        lcl_1 = prim__eq(False, lcl_1)
        lcl_1 = lcl_1 or _tmp_1_flag
        if lcl_1:
            lcl_2 = prim__reset(prim__tokens, _off_1)
            lcl_3 = prim__to__any(prim__nil)
            lcl_3 = (False, lcl_3)
            lcl_1 = lcl_3
        else:
            lcl_1 = _tmp_1_result
        lcl_0 = lcl_1
    return lcl_0
def lr_loop_Mul(_slot_0, prim__state, prim__tokens):
    lr_Mul_reduce = _slot_0
    lcl_0 = prim__tokens.offset
    _off_0 = lcl_0
    lcl_0 = lr_step_Mul(lr_Mul_reduce, prim__state, prim__tokens)
    lr_Mul_try = lcl_0
    lcl_0 = lr_Mul_try[0]
    lcl_0 = prim__not__eq(lcl_0, False)
    while lcl_0:
        lcl_0 = prim__tokens.offset
        _off_0 = lcl_0
        lcl_0 = lr_Mul_try[1]
        lcl_0 = prim__to__result(lcl_0)
        lr_Mul_reduce = lcl_0
        lcl_0 = lr_step_Mul(lr_Mul_reduce, prim__state, prim__tokens)
        lr_Mul_try = lcl_0
        # recalculate condition
        lcl_0 = lr_Mul_try[0]
        lcl_0 = prim__not__eq(lcl_0, False)
    lcl_0 = prim__reset(prim__tokens, _off_0)
    return lr_Mul_reduce
def parse_Atom(prim__state, prim__tokens):
    _tmp_0_flag = False
    lcl_0 = prim__tokens.offset
    _off_0 = lcl_0
    lcl_0 = prim__peekable(prim__tokens, 0)
    if lcl_0:
        # switch
        lcl_2 = prim__peek(prim__tokens, 0)
        lcl_2 = lcl_2.idint
        if lcl_2 == prim__tk__id("quote ("):
            lcl_2 = prim__tokens.offset
            _off_1 = lcl_2
            lcl_2 = prim__tk__id("quote (")
            lcl_2 = prim__match__tk(prim__tokens, lcl_2)
            _slot_0 = lcl_2
            lcl_2 = prim__is__null(_slot_0)
            if lcl_2:
                lcl_3 = (_off_1, "quote (")
                lcl_3 = prim__cons(lcl_3, prim__nil)
                lcl_3 = prim__to__any(lcl_3)
                lcl_3 = (False, lcl_3)
                lcl_2 = lcl_3
            else:
                lcl_3 = parse_Mul(prim__state, prim__tokens)
                _slot_1_check = lcl_3
                lcl_3 = _slot_1_check[0]
                lcl_3 = prim__eq(lcl_3, False)
                if lcl_3:
                    lcl_3 = _slot_1_check
                else:
                    lcl_3 = _slot_1_check[1]
                    lcl_3 = prim__to__result(lcl_3)
                    _slot_1 = lcl_3
                    Atom_a_2 = _slot_1
                    lcl_3 = prim__tokens.offset
                    _off_2 = lcl_3
                    lcl_3 = prim__tk__id("quote )")
                    lcl_3 = prim__match__tk(prim__tokens, lcl_3)
                    _slot_2 = lcl_3
                    lcl_3 = prim__is__null(_slot_2)
                    if lcl_3:
                        lcl_4 = (_off_2, "quote )")
                        lcl_4 = prim__cons(lcl_4, prim__nil)
                        lcl_4 = prim__to__any(lcl_4)
                        lcl_4 = (False, lcl_4)
                        lcl_3 = lcl_4
                    else:
                        lcl_4 = (_slot_0, _slot_1, _slot_2)
                        _slot_local__1 = lcl_4
                        _slot_local__2 = Atom_a_2
                        lcl_4 = (True, _slot_local__2)
                        lcl_3 = lcl_4
                lcl_2 = lcl_3
            lcl_1 = lcl_2
        elif lcl_2 == prim__tk__id("number"):
            lcl_2 = prim__tokens.offset
            _off_1 = lcl_2
            lcl_3 = prim__tk__id("number")
            lcl_2 = prim__match__tk(prim__tokens, lcl_3)
            _slot_0 = lcl_2
            lcl_2 = prim__is__null(_slot_0)
            if lcl_2:
                lcl_3 = (_off_1, "number")
                lcl_3 = prim__cons(lcl_3, prim__nil)
                lcl_3 = prim__to__any(lcl_3)
                lcl_3 = (False, lcl_3)
                lcl_2 = lcl_3
            else:
                Atom_a_2 = _slot_0
                lcl_3 = unwrap(Atom_a_2)
                _slot_local__1 = lcl_3
                lcl_3 = (True, _slot_local__1)
                lcl_2 = lcl_3
            lcl_1 = lcl_2
        else:
            _tmp_0_flag = True
            lcl_2 = prim__to__any(prim__nil)
            lcl_2 = (False, lcl_2)
            lcl_1 = lcl_2
        lcl_0 = lcl_1
    else:
        lcl_1 = prim__to__any(prim__nil)
        lcl_2 = (False, lcl_1)
        lcl_0 = lcl_2
    _tmp_0_result = lcl_0
    lcl_0 = _tmp_0_result[0]
    lcl_0 = prim__eq(False, lcl_0)
    lcl_0 = lcl_0 or _tmp_0_flag
    if lcl_0:
        lcl_1 = prim__reset(prim__tokens, _off_0)
        lcl_2 = prim__to__any(prim__nil)
        lcl_2 = (False, lcl_2)
        lcl_0 = lcl_2
    else:
        lcl_0 = _tmp_0_result
    return lcl_0
def parse_Mul(prim__state, prim__tokens):
    _tmp_0_flag = False
    lcl_0 = prim__tokens.offset
    _off_0 = lcl_0
    lcl_0 = prim__peekable(prim__tokens, 0)
    if lcl_0:
        # switch
        lcl_2 = prim__peek(prim__tokens, 0)
        lcl_2 = lcl_2.idint
        if lcl_2 == prim__tk__id("quote ("):
            lcl_2 = prim__tokens.offset
            _off_1 = lcl_2
            lcl_2 = prim__tk__id("quote (")
            lcl_2 = prim__match__tk(prim__tokens, lcl_2)
            _slot_0 = lcl_2
            lcl_2 = prim__is__null(_slot_0)
            if lcl_2:
                lcl_3 = (_off_1, "quote (")
                lcl_3 = prim__cons(lcl_3, prim__nil)
                lcl_3 = prim__to__any(lcl_3)
                lcl_3 = (False, lcl_3)
                lcl_2 = lcl_3
            else:
                lcl_3 = parse_Mul(prim__state, prim__tokens)
                _slot_1_check = lcl_3
                lcl_3 = _slot_1_check[0]
                lcl_3 = prim__eq(lcl_3, False)
                if lcl_3:
                    lcl_3 = _slot_1_check
                else:
                    lcl_3 = _slot_1_check[1]
                    lcl_3 = prim__to__result(lcl_3)
                    _slot_1 = lcl_3
                    Mul_a_2 = _slot_1
                    lcl_3 = prim__tokens.offset
                    _off_2 = lcl_3
                    lcl_3 = prim__tk__id("quote )")
                    lcl_3 = prim__match__tk(prim__tokens, lcl_3)
                    _slot_2 = lcl_3
                    lcl_3 = prim__is__null(_slot_2)
                    if lcl_3:
                        lcl_4 = (_off_2, "quote )")
                        lcl_4 = prim__cons(lcl_4, prim__nil)
                        lcl_4 = prim__to__any(lcl_4)
                        lcl_4 = (False, lcl_4)
                        lcl_3 = lcl_4
                    else:
                        lcl_4 = (_slot_0, _slot_1, _slot_2)
                        _slot_local__1 = lcl_4
                        _slot_local__2 = Mul_a_2
                        Mul_a_3 = _slot_local__2
                        _slot_local__3 = Mul_a_3
                        lcl_4 = lr_loop_Mul(_slot_local__3, prim__state, prim__tokens)
                        lcl_4 = (True, lcl_4)
                        lcl_3 = lcl_4
                lcl_2 = lcl_3
            lcl_1 = lcl_2
        elif lcl_2 == prim__tk__id("number"):
            lcl_2 = prim__tokens.offset
            _off_1 = lcl_2
            lcl_3 = prim__tk__id("number")
            lcl_4 = prim__match__tk(prim__tokens, lcl_3)
            _slot_0 = lcl_4
            lcl_2 = prim__is__null(_slot_0)
            if lcl_2:
                lcl_2 = (_off_1, "number")
                lcl_2 = prim__cons(lcl_2, prim__nil)
                lcl_2 = prim__to__any(lcl_2)
                lcl_2 = (False, lcl_2)
            else:
                Mul_a_2 = _slot_0
                lcl_2 = unwrap(Mul_a_2)
                _slot_local__1 = lcl_2
                Mul_a_2 = _slot_local__1
                _slot_local__2 = Mul_a_2
                lcl_2 = lr_loop_Mul(_slot_local__2, prim__state, prim__tokens)
                lcl_2 = (True, lcl_2)
            lcl_1 = lcl_2
        else:
            _tmp_0_flag = True
            lcl_2 = prim__to__any(prim__nil)
            lcl_2 = (False, lcl_2)
            lcl_1 = lcl_2
        lcl_0 = lcl_1
    else:
        lcl_1 = prim__to__any(prim__nil)
        lcl_2 = (False, lcl_1)
        lcl_0 = lcl_2
    _tmp_0_result = lcl_0
    lcl_0 = _tmp_0_result[0]
    lcl_0 = prim__eq(False, lcl_0)
    lcl_0 = lcl_0 or _tmp_0_flag
    if lcl_0:
        lcl_1 = prim__reset(prim__tokens, _off_0)
        lcl_2 = prim__to__any(prim__nil)
        lcl_2 = (False, lcl_2)
        lcl_0 = lcl_2
    else:
        lcl_0 = _tmp_0_result
    return lcl_0
