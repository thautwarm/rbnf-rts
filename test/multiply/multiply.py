...
(_, run_lexer) = lexer(r(number='[+-]?\\d+'), r(space='\\s+'), l['*'], l['('], l[')'], ignores=['space'], reserved_map=ImmutableMap.from_dict({'*': 'quote *', '(': 'quote (', ')': 'quote )'}), numbering={'BOF': 0, 'EOF': 1, 'quote *': 2, 'quote (': 3, 'quote )': 4, 'number': 5, 'space': 6})



def mk_parser(mul, unwrap):
